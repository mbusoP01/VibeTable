/* VibeTable Logic - Mobile Ghost Fix & Custom Colors */
...
async function handleLogin() { 
    ...
    // --- SECURE LOADING: ONLY MBUSO GETS THE DATA ---
...
/* --- TIMETABLE LOGIC (GHOST COLUMN + CLICK FIX) --- */
...
function renderTimetable() {
    ...
    // Safety Force Style: Ensure 7 Columns if CSS lags
    if(!sourceData.showWeekends) grid.style.gridTemplateColumns = "60px 40px repeat(5, minmax(140px, 1fr))";
    ...
    days.forEach((d, index) => { 
        ...
        if(index === 1) div.className = 'ghost-col sticky-col'; 
        div.innerText = d; 
