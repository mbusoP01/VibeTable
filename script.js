...
let friendData = null; // Stores friend's timetable temporarily
...
window.onload = function() {
    checkSession(); // 1. Check if we should auto-load or reset
    document.addEventListener('click', updateActivity);
    document.addEventListener('keypress', updateActivity);
    
    initGoogleAuth(); initScrollPickers(); renderColorPickers(); setupDragDrop(); initHeatmap();
    setInterval(updateTimeLine, 60000); setInterval(updateDashboard, 60000);
    if(appData.timerTarget) checkTimerState();
};

/* --- SESSION LOGIC (30 MIN) --- */
function checkSession() {
    const lastActive = localStorage.getItem('vibetable_last_active');
    const now = Date.now();
    
    // If inactive for > 30 mins (30 * 60 * 1000)
    if (lastActive && (now - lastActive > 1800000)) {
        alert("Session expired due to inactivity.");
        localStorage.removeItem('vibetable_data'); // Clear cached sensitive data
        document.getElementById('login-screen').style.display = 'flex';
        document.getElementById('app-screen').classList.add('hidden');
    } else {
        loadFromLocal(); // Safe to load
    }
    updateActivity();
}

function updateActivity() {
    localStorage.setItem('vibetable_last_active', Date.now());
}

/* --- FRIEND & PDF LOGIC --- */
function downloadPDF() {
    const element = document.getElementById('capture-area'); // Target the grid + line
    // Force specific styling for capture
...
function exportSchedule() {
    // Creates a mini JSON just for the timetable
    const shareData = {
        userName: appData.userName,
        timetable: appData.timetable,
        timetableColors: appData.timetableColors,
        startHour: appData.startHour,
        endHour: appData.endHour
    };
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(shareData));
    const node = document.createElement('a');
    node.setAttribute("href", dataStr);
    node.setAttribute("download", `${appData.userName || 'My'}_Schedule.vibe.json`);
    document.body.appendChild(node);
    node.click();
    node.remove();
}

function importFriendSchedule(input) {
    const file = input.files[0];
    if(!file) return;
    const reader = new FileReader();
    reader.onload = function(e) {
        friendData = JSON.parse(e.target.result);
        isViewingFriend = true;
        document.getElementById('friend-mode-banner').style.display = 'flex';
        document.getElementById('friend-mode-banner').querySelector('strong').innerText = `Viewing: ${friendData.userName}'s Schedule`;
        renderTimetable(); // Will now render friendData
    };
...
function exitFriendMode() {
    ...
    friendData = null;
    document.getElementById('friend-mode-banner').style.display = 'none';
    renderTimetable(); // Renders my data
}
...
function renderHabits() {
    ...
    appData.habits.forEach((h, i) => {
        ...
        div.onclick = () => { if(h.last === today) { h.last = null; h.streak--; } else { h.last = today; h.streak++; } triggerSync(); renderHabits(); };
        c.appendChild(div);
    });
}
...
function checkTimerState() {
    ...
    timerInterval = setInterval(() => {
        const now = Date.now(); const left = Math.round((appData.timerTarget - now) / 1000);
        if (left <= 0) { triggerAlarm(); } else { let m = Math.floor(left / 60); let s = left % 60; document.getElementById('timer-display').innerText = `${m}:${s < 10 ? '0' : ''}${s}`; }
    }, 1000);
}
...
/* --- TIMETABLE (WITH SHARED VIEW SUPPORT) --- */
...
function renderTimetable() {
    ...
    // DETERMINE DATA SOURCE (MINE vs FRIEND)
    ...
    for (let i = sourceData.startHour; i <= sourceData.endHour; i++) {
        ...
        for (let j = 0; j < cols; j++) {
            ...
            input.placeholder = isViewingFriend ? '' : '+'; // Hide placeholder in read-only
            if(isViewingFriend) input.setAttribute('readonly', true); // Read Only Mode
            
            input.onchange = (e) => { 
                if(!isViewingFriend) { appData.timetable[key] = e.target.value; triggerSync(); } 
            };
            slot.onclick = (e) => { 
                if(!isViewingFriend && selectedColor && e.target !== input) { 
                    if(!appData.timetableColors) appData.timetableColors = {}; 
                    appData.timetableColors[key] = selectedColor; slot.style.background = selectedColor; triggerSync(); 
                } 
            };
...
async function handleLogin() { 
    ...
    let res = await fetch('https://www.googleapis.com/oauth2/v3/userinfo', {headers:{'Authorization':'Bearer '+accessToken}}); let user = await res.json(); const sidebarPic = document.getElementById('sidebar-pic'); if(sidebarPic) sidebarPic.src = user.picture; 
    await loadData(); if(appData.userName) document.getElementById('dash-name').innerText = appData.userName; updateSyncUI(); triggerSync(); 
}
const CURRENT_FILE = 'vibetable_v13.json'; 
async function loadData() { try { let q = "https://www.googleapis.com/drive/v3/files?spaces=appDataFolder&q=name='" + CURRENT_FILE + "'"; let r = await fetch(q, {headers:{'Authorization':'Bearer '+accessToken}}); let d = await r.json(); if(d.files && d.files.length > 0) { let f = await fetch(`https://www.googleapis.com/drive/v3/files/${d.files[0].id}?alt=media`, {headers:{'Authorization':'Bearer '+accessToken}}); appData = await f.json(); saveToLocal(); renderTimetable(); } } catch(e) { console.error(e); } }
