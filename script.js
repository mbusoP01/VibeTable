/* VibeTable Logic - Offline & Sleep Fixes */
...
window.onload = function() {
    loadFromLocal(); // 1. LOAD INSTANTLY FROM CACHE
    initGoogleAuth(); // 2. TRY AUTH IN BACKGROUND
    initScrollPickers();
    renderColorPickers();
    setupDragDrop();
    initHeatmap();
...
function loadFromLocal() {
    ...
    if (local) {
        ...
        renderGroups(); renderTimetable(); renderEvents(); renderNotes(); updateProfileUI(); updateDashboard(); initHeatmap();
    }
}
...
async function triggerSync() {
    ...
    if(accessToken) {
        ...
        handleAuthClick(); // Prompt login if needed
    }
}
...
/* --- TIMER FIX (SLEEP PROOF) --- */
let timerInterval; 
let targetTime = null;

function startTimer() {
    if(timerInterval) return;
    const durationMinutes = 25; // Default for now
    ...
    timerInterval = setInterval(() => {
        const now = Date.now();
        const left = Math.round((targetTime - now) / 1000);
...
function resetTimer() { pauseTimer(); targetTime = null; document.getElementById('timer-display').innerText = "25:00"; }
...
function updateDashboard() {
    ...
    if(hr >= 10 && hr < 15) verseType = 'midday'; if(hr >= 15 && hr < 20) verseType = 'evening'; if(hr >= 20 || hr < 5) verseType = 'night';
...
function modifyTimetable(end, action) {
    if (action === 'add') { if(end === 'start' && appData.startHour > 0) appData.startHour--; if(end === 'end' && appData.endHour < 24) appData.endHour++; } 
    else { if(end === 'start' && appData.startHour < appData.endHour) appData.startHour++; if(end === 'end' && appData.endHour > appData.startHour) appData.endHour--; }
    triggerSync(); renderTimetable();
}
...
function updateTimeLine() {
    ...
    const headerHeight = 40; const rowHeight = 61; 
    ...
    if (pixelsDown < headerHeight) pixelsDown = headerHeight; if (pixelsDown > maxPixels) pixelsDown = maxPixels; 
    line.style.top = `${pixelsDown}px`; line.style.display = 'block';
}
...
const CURRENT_FILE = 'vibetable_v12.json'; 
